...
class QuotesViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.leftBarButtonItem =
            UIBarButtonItem(barButtonSystemItem: .add, target: self, action: #selector(addQuote))
        navigationItem.rightBarButtonItem =
            UIBarButtonItem(title: "Random", style: .plain, target: self, action: #selector(showRandomQuote))
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let showQuote =
                storyboard?.instantiateViewController(withIdentifier: "ShowQuoteViewController") as? ShowQuoteViewController else {
    ...
    @objc func addQuote() {
        ...
        guard let editQuote =
                storyboard?.instantiateViewController(withIdentifier: "EditQuoteViewController") as? EditQuoteViewController else {
    ...
    override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        let edit =
            UITableViewRowAction(style: .normal, title: "Edit") {[unowned self] (_, indexPath) in
            let quote = self.model.quote(at: indexPath.row)
            self.selectedRow = indexPath.row

            guard let editQuote = self.storyboard?.instantiateViewController(withIdentifier: "EditQuoteViewController") as? EditQuoteViewController
            else {
                SwiftyBeaver.error("Unable to load EditQuoteViewController")
                fatalError("Unable to load EditQuoteViewController")
            }
